...
class AvailableTimesService {
    ...
    @Autowired
    public AvailableTimesService(ApiCallsSenderService senderService, @Value("${SERVICE.ADDRESS.CALENDAR}") String serviceAddress) {
    ...
    private List<String> parseAvailableTimesResponse(ResponseWrapper response) {
        NodeList nodes = response.getNodeListAttribute(START_TIME);
