package au.gov.dto.dibp.appointments.wallet;

import org.junit.Test;

import static au.gov.dto.dibp.appointments.wallet.PushNotificationClient.REJECTION_REASON_PUSH_NOTIFICATIONS_DISABLED;
import static org.hamcrest.Matchers.equalTo;
import static org.junit.Assert.assertThat;

public class PushNotificationClientTest {
    // The following are dummy values that cannot be used to connect to APNs
    private static final String PASS_TYPE_IDENTIFIER = "passTypeIdentifier";
    private static final String PRIVATE_KEY_P_12_BASE_64 = "";
    private static final String PRIVATE_KEY_PASS_PHRASE = "test";

    @Test
    public void pushNotificationsAreDisabledAndMethodsReturnStubAnswersWhenEnvVarIsFalse() throws Exception {
        String pushNotifiationsEnabled = "false";

        PushNotificationClient client = new PushNotificationClient(PASS_TYPE_IDENTIFIER, PRIVATE_KEY_P_12_BASE_64, PRIVATE_KEY_PASS_PHRASE, pushNotifiationsEnabled);

        assertThat(client.isPushNotificationsEnabled(), equalTo(false));
        client.connect();  // should not throw exception
        PushNotificationResponse pushNotificationResponse = client.sendPushNotification("pushToken");
        assertThat(pushNotificationResponse.isAccepted(), equalTo(false));
        assertThat(pushNotificationResponse.isTokenInvalid(), equalTo(true));
        assertThat(pushNotificationResponse.getRejectionReason(), equalTo(REJECTION_REASON_PUSH_NOTIFICATIONS_DISABLED));
        client.disconnect();  // should not throw exception
    }

    @Test
    public void pushNotificationsAreEnabledWhenEnvVarIsNull() throws Exception {
        String pushNotifiationsEnabled = null;

        PushNotificationClient client = new PushNotificationClient(PASS_TYPE_IDENTIFIER, PRIVATE_KEY_P_12_BASE_64, PRIVATE_KEY_PASS_PHRASE, pushNotifiationsEnabled);

        assertThat(client.isPushNotificationsEnabled(), equalTo(true));
    }

    @Test
    public void pushNotificationsAreEnabledWhenEnvVarIsEmptyString() throws Exception {
        String pushNotifiationsEnabled = "";

        PushNotificationClient client = new PushNotificationClient(PASS_TYPE_IDENTIFIER, PRIVATE_KEY_P_12_BASE_64, PRIVATE_KEY_PASS_PHRASE, pushNotifiationsEnabled);

        assertThat(client.isPushNotificationsEnabled(), equalTo(true));
    }

    @Test
    public void pushNotificationsAreEnabledWhenEnvVarIsBlankString() throws Exception {
        String pushNotifiationsEnabled = " ";

        PushNotificationClient client = new PushNotificationClient(PASS_TYPE_IDENTIFIER, PRIVATE_KEY_P_12_BASE_64, PRIVATE_KEY_PASS_PHRASE, pushNotifiationsEnabled);

        assertThat(client.isPushNotificationsEnabled(), equalTo(true));
    }

    @Test
    public void pushNotificationsAreEnabledWhenEnvVarIsTrueLowercase() throws Exception {
        String pushNotifiationsEnabled = "true";

        PushNotificationClient client = new PushNotificationClient(PASS_TYPE_IDENTIFIER, PRIVATE_KEY_P_12_BASE_64, PRIVATE_KEY_PASS_PHRASE, pushNotifiationsEnabled);

        assertThat(client.isPushNotificationsEnabled(), equalTo(true));
    }

    @Test
    public void pushNotificationsAreEnabledWhenEnvVarIsTrueUppercase() throws Exception {
        String pushNotifiationsEnabled = "TRUE";

        PushNotificationClient client = new PushNotificationClient(PASS_TYPE_IDENTIFIER, PRIVATE_KEY_P_12_BASE_64, PRIVATE_KEY_PASS_PHRASE, pushNotifiationsEnabled);

        assertThat(client.isPushNotificationsEnabled(), equalTo(true));
    }
}
